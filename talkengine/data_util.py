...
mod_black = importlib.import_module('model.blacklist_dao')
BlacklistDao = mod_black.BlacklistDao
...
class SlangDataController(DataController):
    def __init__(self):
        self.init_setup()
    ...
    def _get_query_response(self):
        query_response = BlacklistDao.get_entire_blacklist_query_response()

        query_class, reaction_query = [], []
        for res in query_response[0]:
            query_class.append(res['blacklist_id'])
            reaction_query.append(normalize_text(res['text']))

        response_class, reaction_response = [], []
        for res in query_response[1]:
            response_class.append(res['blacklist_id'])
            reaction_response.append(res['text'])
        return (query_class, reaction_query), (response_class, reaction_response)
